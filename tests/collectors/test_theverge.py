...
from src.collectors.theverge import collect
...
@patch("src.collectors.theverge.feedparser.parse")
def test_returns_articles(mock_parse):
    mock_parse.return_value = _mock_feed([
        _make_entry("OpenAI launches new model", "https://www.theverge.com/ai/openai"),
    ])
    ...
    assert articles[0].source == "theverge"


@patch("src.collectors.theverge.feedparser.parse")
def test_caps_at_max_articles(mock_parse):
    entries = [_make_entry(f"AI Story {i}", f"https://www.theverge.com/{i}") for i in range(10)]
...
def test_skips_entry_without_title(mock_parse):
    mock_parse.return_value = _mock_feed([
        _make_entry("", "https://www.theverge.com/ai/article"),
    ])
